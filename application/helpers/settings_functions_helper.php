...
function roll_list_using_exam_id($exm_id, $sel=0) {
	$roll_list_class = $this->db->query("SELECT `roll_no` FROM `exam`, `students` WHERE `exam`.`class_id` = `students`.`class_id` AND `exam`.`grp_id` = `students`.`group_id`  AND `exam`.`exm_id` = '$exm_id'");
	$option = '';
...
function get_location($per_id, $sel=0) {
	...
	$ci->load->database();
	$output = '';
	print "SELECT `lo_id`,`name` FROM `location` WHERE `per_id` = '$per_id'";
...
function get_commission_by_id($user_id) {
	...
	$commission = empty($query->row()->commission) ? "0" : $query->row()->commission;
	return $commission;
}
...
//        $pro_image = mysql_fetch_array(mysql_query("SELECT `photo` FROM `users` WHERE `ID` = '$user_id'"));
    ...
    $ci->load->database();
    $ci->db->select('*');
    $user_info = $ci->db->get_where('users', array('ID' => $user_id))->result();
    if(!empty($user_info)) {
